    ...
    for i in range(len(x)):  # for every sen
        for j in range(len(x[0])):  # for every word
            if x[i][j] == 0 or y[i][j] == 0:
                continue
            if len(id2tag[y[i][j]]) > 0:
                if id2tag[y[i][j]][0] == 'B':
                    entity = [id2word[x[i][j]] + '/' + id2tag[y[i][j]]]
                elif id2tag[y[i][j]][0] == 'M' and len(entity) != 0 and entity[-1].split('/')[1][1:] == id2tag[y[i][j]][
                                                                                                        1:]:
                    entity.append(id2word[x[i][j]] + '/' + id2tag[y[i][j]])
                elif id2tag[y[i][j]][0] == 'E' and len(entity) != 0 and entity[-1].split('/')[1][1:] == id2tag[y[i][j]][
                                                                                                        1:]:
                    entity.append(id2word[x[i][j]] + '/' + id2tag[y[i][j]])
                    entity.append(str(i))
                    entity.append(str(j))
...
def write_entity(outp, x, y, id2tag):
    ...
    for i in range(len(x)):
        ...
        if id2tag[y[i]][0] == 'B':
            ...
            entity += x[i]
            '''line = re.split('[:]', entity.strip())
            index1=x.index(line[1][0])
            index2=x.index(line[1][len(line[1])-1])
            #outp.write(entity + ' ')
            outp.write(id2tag[y[i]][2:] + ':'+str(index1) + ';'+str(index2))'''
            outp.write(entity + '$')
            entity = ''
        else:
            entity = ''
